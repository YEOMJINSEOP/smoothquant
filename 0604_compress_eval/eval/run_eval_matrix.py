...
EVALDIR = os.path.dirname(os.path.abspath(__file__))   # compress_eval/eval (consolidated eval scripts)
PYTHON = sys.executable

GROUPS = ["ruler", "gsm8k", "hotpot"]          # ruler first = heaviest first
# method -> (script, cwd, variant, extra PYTHONPATH). The consolidated eval scripts
# self-bootstrap their sys.path (../shared + absolute /SSD/JSY libs), so cwd/pypath here
# are just a safety net.
METHODS = {
    "original": (f"{EVALDIR}/eval_fp8.py",  EVALDIR, "original", SQ),
    "sq":       (f"{EVALDIR}/eval_w8a8.py", EVALDIR, "combined", SQ),
    "awq":      (f"{EVALDIR}/eval_awq.py",  EVALDIR, "combined", f"{AWQ}:{SQ}"),
    "fp8":      (f"{EVALDIR}/eval_fp8.py",  EVALDIR, "combined", SQ),
}
...
def main():
    ...
    ap.add_argument("--results", default=os.path.join(EVALDIR, "results_matrix"))
